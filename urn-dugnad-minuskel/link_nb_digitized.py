...
import requests
import time
from saturn.urn_service import UrnService
...
urn_service = UrnService(
    url='https://www.nb.no/idtjeneste/ws?wsdl',
    series='URN:NBN:no',
    username='anonymous',
    password='anonymous',
)
log.info("Ok, URN service OK")
...
to_verify = [
    ...
    '998030193514702204',
    '998870356474702204',
    # -- '990934422624702204',  http://urn.nb.no/URN:NBN:no-29532 not found
    # -- '999600196004702204',  not found
]

for mms_id in to_verify:
    log.info("Checking %s", mms_id)
    res = requests.get('https://ub-lsm.uio.no/alma/records/%s?raw=false&expand_items=false&nz=false' % mms_id)
    data = res.json()
    if 'fulltext' in data and data['fulltext'] is not None:
        url = data['fulltext']
        for k in allrows:
            if k[0] == mms_id:
                urn = k[1]
                log.info('Trying to link %s → %s', urn, url)
                current_url = urn_service.get_url(urn)
                urn_service.update(urn, current_url, url)
                log.info('%s: Target URL updated from %s to %s', urn, current_url, url)
                time.sleep(3)
    else:
        log.warning('MMS ID %s NOT FOUND', mms_id)
